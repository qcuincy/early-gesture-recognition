...
class TransformerModel(nn.Module):
    """
    TransformerModel class implements a transformer-based model for gesture recognition.

    Args:
        num_features (int): The number of input features.
        feature_ntokens (List[int]): A list of integers representing the number of tokens for each feature.
    ...
    Attributes:
        ...
        feature_embeddings (nn.ModuleList): A list of embedding layers for each feature.
        encoder (nn.Linear): The linear layer for encoding the input data.
        decoder (nn.Linear): The linear layer for decoding the output data.
        d_model (int): The dimensionality of the model.
        num_features (int): The number of input features.
        feature_ntokens (List[int]): A list of integers representing the number of tokens for each feature.
        src_mask (torch.Tensor): The source mask for the transformer encoder.

    Methods:
        init_weights(): Initializes the weights of the model.
        _generate_square_subsequent_mask(sz): Generates a square subsequent mask.
        forward(src): Performs forward pass through the model.
    ...
    def forward(self, src):
        ...
        Args:
            src (torch.Tensor): The input tensor of shape (sequence_length, batch_size, num_features).

        Returns:
            List[torch.Tensor]: A list of output tensors, each representing a separate feature.
